...
async function likeFood(req, res) {
  try {
    ...
    if (existingLike) {
      await likeModel.deleteOne({ _id: existingLike._id });
      await foodModel.findByIdAndUpdate(foodId, { $inc: { likeCount: -1 } });
      liked = false; // User unliked
    } else {
      await likeModel.create({ user: userId, food: foodId });
      await foodModel.findByIdAndUpdate(foodId, { $inc: { likeCount: 1 } });
      liked = true; // User liked
    }

    // ✅ FIXED: Send proper response that frontend expects
    res.status(200).json({ 
      message: liked ? "Liked successfully" : "Unliked successfully",
      like: liked  // This is what frontend checks: res.data.like
    });
...
async function saveFood(req, res) {
  try {
    ...
    if (existingSave) {
      await saveModel.deleteOne({ _id: existingSave._id });
      await foodModel.findByIdAndUpdate(foodId, { $inc: { savesCount: -1 } });
      saved = false; // User unsaved
    } else {
      await saveModel.create({ user: userId, food: foodId });
      await foodModel.findByIdAndUpdate(foodId, { $inc: { savesCount: 1 } });
      saved = true; // User saved
    }

    // ✅ FIXED: Send proper response that frontend expects
    res.status(200).json({ 
      message: saved ? "Saved successfully" : "Unsaved successfully",
      save: saved  // This is what frontend checks: res.data.save
    });
